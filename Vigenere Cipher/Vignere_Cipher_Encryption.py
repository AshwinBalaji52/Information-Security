...
def get_text(key_list, length_key):
    input_list = []
    get_str = input("Please Input Plain Text A..Z/a..z \n")
...
def encryption(key_result, input_list, length_str):
    ...
    for i in range(length_str):
        character = (ord(input_list[i]) + ord(key_result[i]))%26
        character += ord('A')#this avoid hexadecimal error when we use chr(character) in LOC
        cipher_text.append(chr(character))
    cts = "".join(cipher_text)
    print("\nEncryption Initiated \n")
    tm.sleep(2)
    print("Enciphering ...... \n")
    tm.sleep(4)
    print("Ciphered Text: ",cts)
    return
